...
| ステージ | テーマ | 状態 |
|---------|--------|------|
| **ステージ0** | クラスタを扱える状態にする（Linux・ネットワーク・SSH・Ansible・Docker・監視）| 🚧 進行中 |
| ステージ1 | 並列計算の基本（multiprocessing、mpi4py）| 未着手 |
| ステージ2 | メッセージングとパイプライン（ZeroMQ、MQTT/NATS）| 未着手 |
| ステージ3 | 状態を持つストリーム処理（パーティショニング、時間窓、Redpanda）| 未着手 |
| ステージ4 | 分散フレームワーク（Ray、Dask）| 未着手 |
| ステージ5 | オーケストレーションと耐障害性（k3s）| 未着手 |
| ステージ6 | 分散システムの理論（Raft、CAP、論理時計）| 未着手 |
