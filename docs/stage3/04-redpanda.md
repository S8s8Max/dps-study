...
```
プロデューサー:
  key="pi-cpu"    → murmur2("pi-cpu")    % 3 = 0 → パーティション 0
  key="pi-memory" → murmur2("pi-memory") % 3 = 1 → パーティション 1
  key="pi-temp"   → murmur2("pi-temp")   % 3 = 2 → パーティション 2
```

同じキーのメッセージが同じパーティションに集まる → **状態をパーティション単位で管理できる**

> 📌 振り分けは**クライアント側**で行われ、Kafka 互換クライアントは
> **murmur2** という決まったハッシュ関数を使います。
> Python の組み込み `hash()` ではありません。
>
> 全プロデューサーが同じ関数を使うからこそ、どのプロセスから送っても
> 同じキーが同じパーティションに届きます。
> 自前でキー振り分けを実装するときも、**プロセスをまたいで結果が変わらない
> ハッシュ**（`zlib.crc32` など）を使う必要があります
> — `hash()` は実行ごとに値が変わるため使えません
> （[Stage 4 の解説](../stage4/03-ray-actors.md#組み込みの-hash-を使ってはいけない重要)）。
